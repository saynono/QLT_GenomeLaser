...
PulsePlugin::PulsePlugin(): BasePlugin( "PulsePlugin" ){
}


void PulsePlugin::setup(){
    mBeamDiameter = .01f;
    mBeamDistance = .2f;
...
void PulsePlugin::dispose(){
}


// ------------------------------------------------------------------------------------------------------

const map<string, OSCElement*>& PulsePlugin::getOSCMapping() {
    return mOSCMap;
}

const ColouredShape2d& PulsePlugin::getShape( const GenomeData::BasePairDataSet& dataSet ){
...
void PulsePlugin::drawBeam(Vec2f p, float dia){
    Vec2f pos;
...
void PulsePlugin::drawDescription( Vec2f pos, float len, string description, int descriptionPos ){
